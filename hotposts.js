...
import { getHotposts, invalidateHotpostsCache, getBlockedUserIds, getAcceptedConnections } from './data-layer.js';
...
async function submitHotpost() {
    ...
    try {
        ...
        if (currentMediaType === 'video') {
          ...
          // no client re-encode, so the video-lag instability doesn't apply here at all.
            const filterEffect = FILTER_LIST[capturedFilterIndex].cloudinaryEffect;
            const videoTransform = filterEffect ? `q_auto,vc_auto,f_auto/${filterEffect}` : `q_auto,vc_auto,f_auto`;

        ...
        if (post.post_type === 'text') {
            ...
            if (event) {
                ...
                if (event.show_register_btn && event.register_url) {
                    actionHtml = `<a href="${event.register_url}" onclick="event.preventDefault(); window.openChatLink ? window.openChatLink(this.href) : window.open(this.href, '_blank'); return false;" class="block w-full mt-3 bg-secondary text-white text-center py-2 rounded-xl text-[13px] font-bold active:scale-95 transition-transform">View Link</a>`;

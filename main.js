...
async function updateNativeStatusBar(isDark) {
    try {
        // Same no-bundler fix as openServiceLink's Browser plugin above —
        // and `.isNative` isn't a real Capacitor property (it's the method
        // `.isNativePlatform()`), so this whole block was silently skipped
        // on every native launch before either bug even mattered.
        if (window.Capacitor && window.Capacitor.isNativePlatform()) {
            const StatusBar = window.Capacitor.Plugins && window.Capacitor.Plugins.StatusBar;
            if (!StatusBar) return;
            const bgColor = isDark ? '#121212' : '#f8f9fa';
            const textStyle = isDark ? 'DARK' : 'LIGHT'; // Style.Dark / Style.Light enum values
...
window.openServiceLink = async function(url, openInApp, title) {
    if (!url) return;
    ...
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
        // Native app: use the platform's real in-app browser (Chrome Custom
        // Tabs / SFSafariViewController) — already correct, unchanged.
        if (openInApp) {
            try {
                // No bundler in this deployment (plain <script type="module">,
                // no import map) — `await import('@capacitor/browser')` can't
                // resolve that bare specifier and throws, which was silently
                // falling through to the external-browser branch below EVERY
                // time, regardless of the open_in_app setting. Capacitor's
                // native bridge auto-registers installed plugins on
                // window.Capacitor.Plugins without needing a real ES import —
                // that's the correct no-bundler API and what actually works here.
                const Browser = window.Capacitor.Plugins && window.Capacitor.Plugins.Browser;
                if (!Browser) throw new Error('Browser plugin not registered on native bridge');
